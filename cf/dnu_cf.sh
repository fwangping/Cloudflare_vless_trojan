...
result(){
    ...
    awk -F ',' '$2 ~ /TIA|VIE|MSQ|BRU|SOF|ZAG|LCA|PRG|CPH|TLL|HEL|BOD|LYS|MRS|CDG|TBS|TXL|DUS|FRA|HAM|MUC|STR|ATH|SKG|BUD|KEF|ORK|DUB|MXP|PMO|FCO|RIX|VNO|LUX|KIV|AMS|SKP|OSL|WAW|LIS|OTP|DME|LED|KLD|BEG|BTS|BCN|MAD|GOT|ARN|GVA|ZRH|IST|ADB|KBP|EDI|LHR|MAN/ {print $0}' $ip.csv | sort -t ',' -k5,5n | head -n 10 > EU-$ip.csv

    # 新增动态筛选逻辑
    read -p "是否需要筛选特定地区？请输入机场代码（如 TPE，留空则跳过）： " airport
    if [ ! -z "$airport" ]; then
        # 将输入转为大写
        airport=$(echo "$airport" | tr 'a-z' 'A-Z')

        # 动态筛选输入的机场代码
        awk -F ',' -v code="$airport" '$2 ~ code {print $0}' $ip.csv | sort -t ',' -k5,5n | head -n 10 > ${airport}-$ip.csv
        
        # 检查是否有匹配结果
        if [ -s ${airport}-$ip.csv ]; then
            echo "筛选完成，结果保存在文件：${airport}-$ip.csv"
            cat ${airport}-$ip.csv
        else
            echo "未找到与机场代码 ${airport} 匹配的记录。"
            rm -f ${airport}-$ip.csv # 删除空文件
        fi
    fi
}
